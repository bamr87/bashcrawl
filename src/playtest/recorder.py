...
server, the server is the only component that observes every move — so it owns
the audit log. :class:`SessionRecorder` writes one JSONL line per event and
classifies each turn so the offline scorer (:mod:`playtest.scorer`) needs
...
Ported (TUI-free) from the retired ``ti.playtest.recorder``; the snapshot it
consumes now comes from a real bash session (see
:class:`playtest.bash_session.BashGameSession`), so ``xp`` /
``completed_quest_ids`` are simply absent for the pure-filesystem game and
progress is judged on ``cwd`` / ``inventory`` / ``hp`` / ``room_items``.
"""
...
class SessionRecorder:
    ...
    def record_command(
        ...
        if outcome in (ERROR, REPEAT):
            ...
            # PROGRESS and NEUTRAL both break the "non-productive attempts in a
            # row" streak — only consecutive errors/repeats may open a gap.
            self._struggle_run = 0
            if outcome == PROGRESS:
