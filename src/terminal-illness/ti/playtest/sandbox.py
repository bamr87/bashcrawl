...
This mirrors the proven ``test/fixtures`` sandbox logic but lives in the runtime
package so the MCP server can use it without importing the test tree (which is
excluded from the package). The two are kept deliberately independent: the test
fixture serves the pytest suite, this serves the MCP server / playtest harness.
"""
...
_GAME_STATE_FILES = [
    ...
    # save the source repo carried in. The MCP server also points the session
    # at a dedicated save path, but clearing these keeps the tree honest.
    ".bashcrawl_save.json",
    ".ti_save.json",
]
...
def find_game_root(start: Path | None = None) -> Path:
    ...
    The root is the directory containing both ``entrance/`` and ``main.sh``.
    """
